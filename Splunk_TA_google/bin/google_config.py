...
def setup_signal_handler(data_loader, kconfig):
    ...
    def _handle_exit(signum, frame):
        if kconfig is not None and kconfig.is_dispatcher():
            logger.info("Remove file lock")
            store = create_state_store(
                kconfig._meta_configs, kconfig._appname,
                kconfig._server_info)
...
class KafkaConfig(object):
    _appname = utils.get_appname_from_path(op.abspath(__file__))
    ...
    def _dispatch_tasks(self, global_settings, kafka_clusters):
        ...
        lock = FileLock(self._meta_configs, self._appname, self._server_info)
        if not lock.locked():
            return
    ...
    def _get_task_configs(self, global_settings, kafka_clusters):
        self._remove_tasks(kafka_clusters)
            ...
            for task in forwarder_tasks.itervalues():
                for i, topic in enumerate(task[c.kafka_topic]):
                    if isinstance(topic, str):
                        # old ckpt schema
                        task[c.kafka_topic][i] = [topic, None, index]
    ...
    def _dispatch_tasks_to_forwarders(self, global_settings, kafka_clusters,
        ...
        failed = self._disable_tasks_on_forwarders(kafka_clusters, forwarders,
                                                   dispatched_tasks,
                                                   kafka_topics)
    ...
    def _do_dispatch_tasks(self, task_assignments, dispatched_tasks,
        ...
        changed = self._global_settings_changed(global_settings)
            ...
            for forwarder_stanza, forwarder_task in forwarder_tasks.iteritems():
                if forwarder_stanza not in forwarder_settings:
                    forwarder_settings[forwarder_stanza] = \
                        self._get_settings(changed, global_settings)
    ...
    def _get_data_input_status(dispatched_tasks, forwarder_stanza,
        ...
        if (datainput[c.kafka_partition] == committed_di.get(c.kafka_partition)
                or datainput[c.index] == committed_di.get(c.index)):
            return "update_ckpt"
        ...
        for topic_info in topics:
            if topic_info[0] == datainput[c.kafka_topic]:
                if (topic_info[1] != datainput[c.kafka_partition] or
                        topic_info[-1] != datainput.get(c.index)):
                    return "update"
        return "update"
    ...
    def _do_send_safe(self, clusters, dispatched_tasks, forwarder_stanza,
        ...
        if not localhost and c.global_settings in settings["config_settings"]:
            ...
            if resp is None or resp.status not in (200, 201):
                logger.info("Failed to update config settings on forwarder=%s",
                            forwarder[c.hostname])
                self._dispatch_errors = True
                return
    ...
    def _disable_tasks_on_forwarders(self, kafka_clusters, forwarders,
    ...
    def _disable_tasks_for_deleted_forwarders(self, kafka_clusters, forwarders,
                                              dispatched_tasks,
                                              removed_clusters):
    ...
    def _remove_tasks(self, clusters):
        conf_mgr = cm.ConfManager(self._meta_configs[c.server_uri],
                                  self._meta_configs[c.session_key],
                                  app_name=self._appname)
        ...
        for cluster_stanza, cluster in clusters.iteritems():
            ...
            for stanza in all_stanzas:
                ...
                try:
                    ...
                    has_exception = True
                    logger.error("Failed to remove data input=%s, "
                                 "error=%s", cluster_stanza,
                                 traceback.format_exc())
                    continue

            if not has_exception:
                conf_mgr.set_appname(self._appname)
                conf_mgr.delete_stanza(c.myta_cred_conf, cluster_stanza)
    ...
    @staticmethod
    def _filter_partitions(stanza, topics, cluster, kafka_topics):
        partition_ids = KafkaConfig.get_partitions(stanza)
        if not partition_ids:
            return []

        client = KafkaConfig._get_client(cluster, kafka_topics)
        topic_partitions = client.topic_partitions
        valid_ids = set(partition_ids)
        for topic in topics:
            for pid in partition_ids:
                if pid not in topic_partitions[topic]:
                    if pid in valid_ids:
